...
export class AssistantComponent implements OnInit {
  ...
  isLoading: boolean = false; // <-- ajouté
  ...
  sendMessage(): void {
    ...
    this.assistantService.sendMessage(message).subscribe({
      next: (response) => {
        ...
        this.isLoading = false; // <-- stoppe le loader
      },
      error: (err) => {
  ...
  confirmUpload(): void {
    ...
    this.messages.push({
      text: `Document importé: ${this.selectedFileName}`,
      sender: 'user',
    });
    ...
    setTimeout(() => {
      this.messages.push({
        text:
          "J'ai analysé le document. Vous pouvez maintenant me poser des questions spécifiques sur son contenu.",
        sender: 'bot',
      });
    }, 1500);
